...
class Executor a where
  ...
  initExec :: (MonadIO m, MonadEnv m) => RunCtx ev env opts cfg -> m (Either Text a)
  setupExec ::  (MonadIO m, MonadEnv m, MonadEventLogger m, MonadFileSystemRead m, Show cfg) => RunCtx ev env opts cfg -> a -> m (Either Text a)
  finishExec :: (MonadIO m, MonadEventLogger m) => RunCtx ev env opts cfg -> a -> m (Either Text a)
